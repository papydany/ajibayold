...
$width =$width;
 ...
 echo"<td width='$width%' colspan='3'> <p class='sub_rotate'>",isset($sb[$i]['subject_name']) ? str_replace(" ","",$sb[$i]['subject_name']) : '',"</p></td>";
}?>
...
//$elective_subject = get_all_elective_subject($c,$sc,$v['class_option_id'],$term,$s);
//var_dump($elective_subject);
$c_subject_num = count($compulsary_subject);
//foreach ($compulsary_subject as $key => $value) {

  $c_s_id =$compulsary_subject;//$value['subject_id'];
//}
...
//$total_num_of_student = count($reg2);
...
if($c > 3 && $s < 2017){
  $Tm_first = select_sub_total($v['student_id'],$c,$sc,$c_s_id,'First Term',$s); // total marks function for first term for senior students
   // $Tm_second = select_sub_total($v['student_id'],$c,$sc,$c_s_id,'Second Term',$s);
      $Tm_third = select_sub_total($v['student_id'],$c,$sc,$c_s_id,'Third Term',$s);
}else{
$Tm_first = select_sub_total($v['student_id'],$c,$sc,$sb_id,'First Term',$s); // total marks function for first term
//$Tm_second = select_sub_total($v['student_id'],$c,$sc,$sb_id,'Second Term',$s);
$Tm_third = select_sub_total($v['student_id'],$c,$sc,$sb_id,'Third Term',$s);
}
...
if($s < 2017)
{
 ...
 $av_s =round($Tm_second / $nine,1);
 $av_t =round($Tm_third / $nine,1);
}else
 // from session 2017 new condition
 ...
 $av_s =round($Tm_second / $sub_num,1);
 $av_t =round($Tm_third / $sub_num,1);
 }

  }else{
 $av_f =round($Tm_first / $sb_num,1); // average of subject in first term 
 $av_s =round($Tm_second / $sb_num,1);
 $av_t =round($Tm_third / $sb_num,1);
}
...
for ($i=0; $i < $sub_total; $i++) {
  ...
  $secodsemestercheck =isset($rss[$i]['student_mark']) ? $rss[$i]['student_mark'] : '';
}
...
for ($i=0; $i < $sub_total; $i++) {
 ...
 if(!in_array($rss[$i]['subject_id'], $e2)){
  // subject not in first term
$s_80 =$rss[$i]['student_mark'];
 }else{
  $s_80 =round($rss[$i]['student_mark']* 0.8,1);
 }
...
for ($i=0; $i < $sub_total; $i++) {
  $secodsemestercheck =isset($rss[$i]['student_mark']) ? $rss[$i]['student_mark'] : '';
  $firstsemestercheck =isset($rs[$i]['student_mark']) ? $rs[$i]['student_mark'] : '';
  echo"<td colspan='2'></td>";
   if(!empty($secodsemestercheck))
 {
  $f_20 =round($rs[$i]['student_mark']* 0.2,1);
}else
{
  $f_20 =round($rss[$i]['student_mark']* 0.2,1);
}
...
{
$Rs1 =round($rss[$i]['student_mark'] * 0.2,1);
$Rs2 = round($rss[$i]['student_mark']* 0.8,1);
}
  if(empty($firstsemestercheck))
 {
...
$u =subject_position($c,$sc,$s,$rss[$i]['subject_id'],$term);
 //var_dump($u);
 ...
 if(!empty($u)){
  foreach ($u as $key => $value) {
    ...
    if($v['student_id'] == $id){
     ...
     echo grade($v['student_id'],$c,$sc,$s,$rss[$i]['subject_id'],$term);
...
echo"</tr>";
echo"<tr class='resultbg'><td></td><td colspan='4'><b>Total mark </b></td><td colspan='3'><b>";if($c > 3)
{ if($s < 2017)
  {
  if(!empty($max)){ echo $sec+$max; }else{ echo $sec ;}
}else{
  echo $Totalaverage2;
}

}else{ echo $Totalaverage2;}echo"</b></td>
   <td colspan='3'></td><td colspan='4'><b>Average</b></td><td colspan='3'><b>";
   if($c > 3){
if($s < 2017){
  
    $second_Average = round(($sec + $max)/$nine,1);
}else
{
  $second_Average =  round($Totalaverage2/$sub_num  ,1);
}
}else{ 
  $second_Average =  round($Totalaverage2/$sb_num ,1);}
...
elseif($firsttermcheck == false && $secondtermcheck ==true){  //student not present for second term
...
if(in_array($rsss[$i]['subject_id'],$compulsary_subject )){
  ...
  $total_3 = round($rsss[$i]['student_mark'] * 0.8,1);
  $elect_total_thd= $total_1 + $total_3;
  $elect_sub[] = $elect_total_thd;
...
for ($i=0; $i < $sub_total; $i++) {
  echo"<td colspan='2'></td>";
   if(!in_array($rsss[$i]['subject_id'], $thirdsubject)) // subject not taken in third term
  {
    $S_20 ='';
  }
  else
...
if($c < 4){
  if(in_array($rsss[$i]['subject_id'], $thirdsubject)) // subject not taken in third term
  {
 $u =subject_position($c,$sc,$s,$rsss[$i]['subject_id'],$term);
 //var_dump($u);
 ...
 if(!empty($u)){
    ...
    if(in_array($rsss[$i]['subject_id'], $thirdsubject)) // subject not taken in third term
  {
    ...
    echo grade($v['student_id'],$c,$sc,$s,$rsss[$i]['subject_id'],$term);
  }
  }
...
if($c > 3)
    ...
    if($s < 2017)
  {
    ...
    else{ echo $Totalaverage3;}
    echo"</b></td><td colspan='3'></td>
   <td colspan='4'><b>Average</b></td><td colspan='3'><b>";
   if($c > 3)
    {
      if($s < 2017){
      $third_average = round(($thd + $max)/$nine,1);}
      else
{
  $third_average =  round($Totalaverage3/$sub_num  ,1);
}
}
  else{ $third_average =  round($Totalaverage3/$sb_num ,1);}
  echo $third_average.
    "</b></td><td colspan='3'></td><td colspan='5'><b>Class Position</b></td><td colspan='2'>";
$position = position_first($c,$sc,$s,$term,$v['class_option_id']);
...
<div class="col-sm-3">
  <p>Principal`s Name <b>&nbsp;:&nbsp; MR OKE D. O</b></p>
  <p><img src="../images/principal.jpg" class="img-responsive"/></p>
